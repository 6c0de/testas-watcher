  ...
  try {
    ...
    const rows = await page.$$eval('.row.entry', (elements) =>
      elements.map((el) => {
        ...
        const statusText = statusLink ? statusLink.textContent.trim().toLowerCase() : '';
    ...
    for (const school of TARGET_SCHOOLS) {
      ...
      let status;
      if (row.statusText === 'ausgebucht') {
        status = 'closed';
      } else if (row.statusText === 'anmelden') {
        status = 'open';
      } else {
        throw new Error(
          `Unrecognized status text "${row.statusText}" for ${school} - site markup may have changed`
        );
      }
      result[school] = { status, address: row.address };
    }
    return result;
  } finally {
    await browser.close();
